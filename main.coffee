...
class Swift
  constraints: ($) ->
    if @options.constraints.verticalTop
      ...
      $ "view.addConstraint(NSLayoutConstraint(item: #{@layers[0].name}, attribute: .Top, relatedBy: .Equal, toItem: #{@layers[1].name}, attribute: .Top, multiplier: 1.0, constant: 0.0))"
      $.newline()

    if @options.constraints.verticalCenter
      $ "// center #{@layers[0].name} vertically with #{@layers[1].name}"
      $ "view.addConstraint(NSLayoutConstraint(item: #{@layers[0].name}, attribute: .CenterY, relatedBy: .Equal, toItem: #{@layers[1].name}, attribute: .CenterY, multiplier: 1.0, constant: 0.0))"
      $.newline()

    if @options.constraints.verticalBottom
      $ "// align #{@layers[0].name} and #{@layers[1].name} to bottom"
      $ "view.addConstraint(NSLayoutConstraint(item: #{@layers[0].name}, attribute: .Bottom, relatedBy: .Equal, toItem: #{@layers[1].name}, attribute: .Bottom, multiplier: 1.0, constant: 0.0))"
      $.newline()

    if @options.constraints.horizontalLeft
      $ "// align #{@layers[0].name} and #{@layers[1].name} to left"
      $ "view.addConstraint(NSLayoutConstraint(item: #{@layers[0].name}, attribute: .Left, relatedBy: .Equal, toItem: #{@layers[1].name}, attribute: .Left, multiplier: 1.0, constant: 0.0))"
      $.newline()

    if @options.constraints.horizontalCenter
      $ "// center #{@layers[0].name} horizontally with #{@layers[1].name}"
      $ "view.addConstraint(NSLayoutConstraint(item: #{@layers[0].name}, attribute: .CenterX, relatedBy: .Equal, toItem: #{@layers[1].name}, attribute: .CenterX, multiplier: 1.0, constant: 0.0))"
      $.newline()

    if @options.constraints.horizontalRight
      $ "// align #{@layers[0].name} and #{@layers[1].name} to right"
      $ "view.addConstraint(NSLayoutConstraint(item: #{@layers[0].name}, attribute: .Right, relatedBy: .Equal, toItem: #{@layers[1].name}, attribute: .Right, multiplier: 1.0, constant: 0.0))"
      $.newline()
